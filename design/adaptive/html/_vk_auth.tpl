...
<form autocomplete="off" action="/" method="get" name="form_discount1" id="form_discount1" enctype="multipart/form-data">
			<div class="sett1">
				<div class="ShAA_pop_title">Пожалуйста, войдите как пользователь популярных соцсетей</div>
				{literal}
				<div class="ShAA_socIcons" id="uLogin" data-ulogin="display=panel;fields=first_name,last_name,photo,photo_big,sex;providers=vkontakte,odnoklassniki,mailru,facebook,twitter,google,yandex,livejournal,youtube;hidden=;redirect_uri=http%3A%2F%2F{/literal}{$server_name}{literal}%2Flogin%2F"></div>
				<script type="text/javascript">
					uLogin.extraction();
				</script>
				{/literal}
				<hr color="#dcdcdc" size="1" />
				<div style="margin: 24px 0 0 0;" class="ShAA_pop_title">или как владелец персональной карты Лакшери стор</div>
				...
				<div>
					...
					<div class="ShAA_popData">
						...
						<div class="ShAA_popInput">
							<span class="ShAA_prefix">+7</span><input placeholder="XXXXXXXXXX" type="text" name="phone" id="id_phone" {literal}class="validate[required,custom[phone]]"{/literal} value="{$save_phone_number}"/>
						</div>
				...
				<div style="float: left; margin: 0 0 16px 0; width: 100%;">
					...
					<a href="javascript:void(0);" onclick="jQuery('#form_discount1').submit();return false;">				
						<div class="ShAA_popButtonIn" style="float: left;"></div>
					</a>
					<ul class="tabsSett tabs1" style="float: left; height: auto; margin: 10px 10px 0 35px; width: 200px;">
			...
			<div class="sett2">
				...
				<div class="ShAA_popData">
					...
					<div class="ShAA_popInput">
						<span class="ShAA_prefix">+7</span><input placeholder="XXXXXXXXXX" type="text" name="phone_remember" id="id_phone_remember" {literal}class="validate[required,custom[phone]]"{/literal} value="{$save_phone_number}"/>
					</div>
					<div class="ShAA_popInfoInput">
					</div>
				</div>
