<h1 style='font-weight:normal;font-family:arial;'><a href='https://{$root_url}/{$order->code}'>Заказ №{$order->order_id}</a> на сумму {$order->total_amount} {$main_currency->sign}</h1>
<table cellpadding=6 cellspacing=0 style='border-collapse: collapse;'>
  ...
  <tr>
    ...
    <td style='padding:6px; width:330; background-color:#ffffff; border:1px solid #e0e0e0;font-family:arial;'>
      {if $user}
      <a href='https://{$root_url}/admin/index.php?section=Orders&user_id={$user->user_id}'>{$order->name|escape}</a>
      {else}
      {$order->name|escape}
      {/if}
    </td>
...
<table cellpadding=6 cellspacing=0 style='border-collapse: collapse;'>
  ...
  <tr>
    <td style='padding:6px; width:250; padding:6px; background-color:#f0f0f0; border:1px solid #e0e0e0;font-family:arial;'>
      <a href="//{$root_url}/products/{$product->url}/">
		{if $product->large_image || $product->small_image}<img src="//{$root_url}/reimg/files/products/184x/{if $product->large_image}{$product->large_image}{else}{$product->small_image}{/if}" alt="{$product->model}">
...
<a href='https://{$root_url}/order/{$order->code}'>https://{$root_url}/order/{$order->code}</a>
<br />
Менеджер вашего заказа №{$order->order_id}<br />
Усачев Владимир +79524442636<br />
<a href='https://{$root_url}'>{$settings->site_name}</a>
