...
<body style="font-size: 14px; text-align: center;">
    <div class="ShAA_tableBlock">
        <div style="border-bottom: 1px solid #000; width: 100%;margin-top: 12px;font-size:16px;text-align: left;">{$client_name}</div>
        ...
        <div style="font-weight: bold; font-size: 20px; margin: 24px 0 12px;">
            Акт приёма-передачи №{$order->receipt_number} от {$order->date|date_format:"%d.%m.%Y"}г
        </div>
        <div>
        ...
        {if $order->service_order && $order->service_order->defect_description}
          <div style="float: left;"><b>Описание дефекта</b>: {$order->service_order->defect_description}</div>
        {/if}
