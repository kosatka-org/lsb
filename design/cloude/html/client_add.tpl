			...
			<div>
				...
				<form autocomplete="off" action="/index.php?module=Login&client_add" method="post" name="client_add" id="client_add" enctype="multipart/form-data">
					<div class="sett1">
						...
						<div>
							<div class="ShAA_popData ShAA_popDataSett">
								...
								<div class="ShAA_popInput">
									<input placeholder="Фамилия Имя Отчество" type="text" name="name" id="name" {literal}class="validate[required]"{/literal} value="" />
								</div>
							</div>
							<div class="ShAA_popData ShAA_popDataSett">
								<div class="ShAA_popTitleInput">
							...
							<div class="ShAA_popData ShAA_popDataSett">
								...
								<div class="ShAA_popInput ShAA_personCard">
									<input placeholder="Номер дисконтной карты 16 &ndash; 22 цифр" type="text" name="card_number" id="id_card_number" value="" />
								</div>
							</div>
								...
								<div class="ShAA_popInput">
									<select name="manager_id">
										...
										{foreach from=$managers item=manager}
											<option value="{$manager->user_id}" {if $smarty.session.user->user_id == $manager->user_id}selected{/if}>{$manager->name}</option>
										{/foreach}
									</select>
								</div>
							</div>
							<div class="ShAA_popData ShAA_popDataSett">
								...
								<div class="ShAA_popInputMini">
									<select name="sizetop">
										...
										{foreach from=$top_sizes item=size}
											<option value="{$size->size}">{$size->size} (RU)</option>
										{/foreach}
									</select>
								</div>
								...
								<div class="ShAA_popInputMini">
									<select name="sizebottom">
										<option selected value="">---</option>
										{foreach from=$bottom_sizes item=size}
											<option value="{$size->size}">{$size->size} (RU)</option>
										{/foreach}
									</select>
								</div>
								...
								<div class="ShAA_popInputMini">
									<select name="sizeshoe">
										<option selected value="">---</option>
										{foreach from=$shoe_sizes item=size}
											<option value="{$size->size}">{$size->size} (RU)</option>
										{/foreach}
									</select>
								</div>
                        ...
                        <div class="clear"></div>
                        <div class="ShAA_popMiniInfo" style="margin-bottom: 12px;">Нажимая на кнопку "Сохранить", клиент дает <a href="/sections/personal_data">согласие на обработку персональных данных</a></div>
					</div>
				</form>
				<form autocomplete="off" action="/index.php?module=Cart&client_find&search='+jQuery('#client_info').eq(0).val().replace(/ /g, '+'))" method="post" name="client_find" id="client_find" enctype="multipart/form-data">
					<div class="sett2">
