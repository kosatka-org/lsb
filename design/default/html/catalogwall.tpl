		...
		<div itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product" id="{$product->product_id}" checkBrands="{$product->brand_id}" checkCategories="{$product->category_id}" class="ShAA_catalogItem_new category_{$product->category_id} brand_{$product->brand_id} week_{$product->week} sex_{$product->sex}" {if $product->hidden}style="display:none"{/if}>
			...
			<div id="img_{$product->product_id}" class="imgCatalog_new">
				<a href="/products/{$product->url}/{if $recommended_by}?recommended_by={$recommended_by}{/if}" target="_blank" title="{$product->model} из Италии и Франции" style="border-bottom:none;">
					<img alt="{$product->model} из Италии и Франции" title="{$product->model} из Италии и Франции" src="{if $cdn_image_link}{$cdn_image_link}{else}//lsboutique.ru{/if}/reimg/files/products/184x/{if $product->large_image}{$product->large_image}{else}{if $product->second_image}{$product->second_image}{/if}{/if}" src_out="{if $cdn_image_link}{$cdn_image_link}{else}//lsboutique.ru{/if}/reimg/files/products/184x/{if $product->large_image}{$product->large_image}{else}{if $product->second_image}{$product->second_image}{/if}{/if}" src_over="{if $cdn_image_link}{$cdn_image_link}{else}//lsboutique.ru{/if}/reimg/files/products/184x/{if $product->second_image}{$product->second_image}{else}{if $product->large_image}{$product->large_image}{/if}{/if}" itemprop="image" />
				</a>
			</div>
				...
				<a itemprop="offers" itemscope itemtype="http://schema.org/AggregateOffer" href="/products/{$product->url}/{if $recommended_by}?recommended_by={$recommended_by}{/if}" target="_blank">
					{if $product->can_buy_from_site}
						...
						{elseif $product->old_price != 0 && $product->old_price>$product->price ||  $product->prop_val == 'Sale'}
							<span style="color: #C30000;">{if ($product->price/$product->old_price) < 0.5}последняя цена {/if}<b itemprop="lowPrice">{$product->price|string_format:"%.0f"}</b> {$currency->sign|escape}</span>
							<span>вместо <span itemprop="highPrice">{$product->old_price|string_format:"%.0f"}</span> {$currency->sign|escape} </span><br/>
					    {elseif $product->prop_val == 'Заказ'}
						    {if $product->discount_price && $product->discount_price < $product->price}
							    <span>стоимостью <span itemprop="highPrice">{$product->price|string_format:"%.0f"}</span></span><br/>
					    ...
					    {if $product->show_price && $product->price >= $product->old_price && !$product->no_sale}
						    {if $product->discount_price && $product->discount_price < $product->price}
	        ...
	        {/if}
{if $smarty.session.user->purchase_sum_real > 0 && $new_season }
