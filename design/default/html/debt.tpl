...
<div class="container" style="margin-bottom:40px;">
  <div class="row mt20">
    <div class="col-md-8" id="left-column" style="padding-left: 0;">
      <div class="row">
        Задолженность по покупке №<a href="/index.php?module=OfflineSale&order_id={$debt->order_id}"> {$debt->order_id}</a>,  {$debt->user->name}, общая сумма долга: <b>{$debt->debt_amount|number_format:0:",":" "}</b>₽ ({$debt->cashbox->name})
      </div>
      <h3>Список выплат</h3>
      <div class="row">
      ...
      <div id="payment-list">
        {foreach from=$debt->payments item=payment}
          <div class="row">
            ...
            <div class="col-md-3 mt10"><input class="form-control payment-input" disabled value="{$payment->money_paid|number_format:0:",":" "}" style="width: 100%; text-align: right; padding-right: 3px;"></div>
            <div class="col-md-3 mt10">{$payment->cashbox_name}</div>
            <div class="col-md-3 mt10">{$payment->date|date_format:"%Y/%m/%d, %H:%M"}</div>
          </div>
        {/foreach}
...
{/literal}
{if $payment_options}
  debt.payment_options = JSON.parse('{$payment_options}');
  debt.cashboxes = JSON.parse('{$cashboxes_json}');
