...
$j(document).ready(function() {
    ...
    function startParallax(width, pos) {
		var height = jQuery(window).height();
		jQuery( '.parallax_l' ).width(width);
		jQuery(window).scrollTop(0);
		jQuery( '.parallax_c' ).prepend('<div id="parallax_close"></div>');
		jQuery( '.parallax_l' ).find('img').css({'height':'auto', 'width': (width - 50) + 'px', 'max-height':'2400px', 'max-width':'1600px', 'margin-top':pos, 'margin-left':'50px'});
		jQuery( '.parallax_c' ).css({'position':'fixed','top':0,'left':0});
		jQuery( '.parallax_c' ).width(width).height(height);

		var range = (jQuery('#zoomImg').height())/2 + 300;
		jQuery('.parallax_l').plaxify({"xRange":0,"yRange":range,"invert":true});
		jQuery.plax.enable({ "activityTarget": jQuery('#PlaxDiv')});
		jQuery('.footer, #headBlock').addClass('invisible');
		disableScroll();
	}
	function stopParallax() {
		jQuery( '.parallax_l' ).find('img').removeAttr( 'style' );
		...
		jQuery('.footer, #headBlock').removeClass('invisible');
		enableScroll();
	}
	function resizeParallax(width) {
		if (jQuery("#zoomImg").hasClass('ShAA_zoomImageForProduct')){
			var height = jQuery(window).height(),
				width = jQuery(window).width() - 240;
			jQuery( '.parallax_l' ).width(width - 50);
			jQuery( '.parallax_c' ).width(width).height(height);
			jQuery("#zoomImg").css({'width': (width - 50) + 'px'});
	...
	jQuery(document).on("click", '#zoomImg', function () {
		if(jQuery(window).width() < 1200){return false;}
		var width = jQuery(window).width() - 240;
		jQuery('.parallax_l').plaxify({"xRange":0,"yRange":1500,"invert":true});
		if (jQuery(this).hasClass('ShAA_zoomImageForProduct')){
			stopParallax();
		}
		else{
			startParallax(width, "-48%" );
		}
		jQuery(this).toggleClass('ShAA_zoomImageForProduct');
	});
	jQuery(document).on("click", '#parallax_close', function() {
		stopParallax();
		jQuery("#zoomImg").removeClass('ShAA_zoomImageForProduct');
	});
});
{/literal}
</script>
	        	...
	        	<div class="Product_centerRightContent" itemscope="" itemtype="http://schema.org/Product">
		        			...
		        			<div class="Product_productMiddleImg">
                                 <div class="parallax_c pr_parallax" id="PlaxDiv">
                                    <div class="parallax_l">
                                        <img itemprop="image" class="cloudzoom" id="zoomImg" alt="" src="/files/products/{$product->large_image}" data-zoom-img = "/files/products/{$product->large_image}"
                                        data-cloudzoom = "zoomImage: '/files/products/{$product->large_image}', zoomOffsetX: 90, zoomOffsetY: 0, zoomClass: 'cloudzoom-zoom-big', lensClass: 'cloudzoom-lens',disableZoom: true" />
                                    </div>
                                </div>
		        			</div>
									...
									{if $product->category_id != 8233}
										...
										{if $product->large_image }
											<li>
												...
												<img class="cloudzoom-gallery" alt="{$product->img_desc}" title="{$product->img_desc}" src="/reimg/files/products/85x/{$product->large_image}"
												 data-cloudzoom = "
												 useZoom: '#zoomImg',
												 image:'/reimg/files/products/560x/{$product->large_image}',
												 zoomImage:'/files/products/{$product->large_image}'
												 ">
											</li>
										{/if}
										{if $big_size && $product->bsize_small_image}
											<li>
												<div class="shadow"></div>
												<img class="cloudzoom-gallery" alt="{$product->img_desc}" title="{$product->img_desc}" src="/reimg/files/products/85x/{$product->bsize_small_image}"
												 data-cloudzoom = "
												 useZoom: '#zoomImg',
												 image:'/reimg/files/products/560x/{$product->bsize_small_image}',
												 zoomImage:'/files/products/{$product->bsize_small_image}'
												 ">
											</li>
										{elseif $product->small_image}
											<li>
												<div class="shadow"></div>
												<img class="cloudzoom-gallery" alt="{$product->img_desc}" title="{$product->img_desc}" src="/reimg/files/products/85x/{$product->small_image}"
												 data-cloudzoom = "
												 useZoom: '#zoomImg',
												 image:'/reimg/files/products/560x/{$product->small_image}',
												 zoomImage:'/files/products/{$product->small_image}'
												 ">
											</li>
										{/if}
										{if $product->fotos}
											{foreach from=$product->fotos item=foto}
												<li>
													<div class="shadow"></div>
													<img class="cloudzoom-gallery" alt="{$product->img_desc}" title="{$product->img_desc}" src="/reimg/files/products/85x/{$foto->filename}"
													 data-cloudzoom = "
													 useZoom: '#zoomImg',
													 image:'/reimg/files/products/560x/{$foto->filename}',
													 zoomImage:'/files/products/{$foto->filename}'
													 ">
												</li>
											{/foreach}
										{/if}
	        		...
	        		<div class="Product_rightBlockForProduct" {if $product->category_id == 8233} style="padding-left:  24px;"{/if}>
	        			<div class="productInfo">
	        				<div class="titleMain">
								<img width="212" alt="{$brand->name}" title="{$brand->name}" src="/reimg/files/brands/212x/{$brand->image}">
							</div>
								...
								{if $can_buy_from_site && $show_price == 0}
									{if $product->old_price > $product->price}
										...
										{if ($podium && ($product->old_price / $product->price == 2 )) || ($brand->no_sale == 1)}
                                            <span style="text-decoration: line-through;" itemprop="highPrice">{$product->old_price|string_format:"%.0f"}&nbsp;<i class="icon-rub"></i></span><br/>
                                            <span style="color: #C30000; width: 100%;"><span style="font-size: 18px; float: none; font-weight: bold;" itemprop="lowPrice">{$product->price|string_format:"%.0f"}&nbsp;<i class="icon-rub"></i></span> со скидкой {$product->sale_value|string_format:"%.0f"}%</span> </br>
...
-->
                                <a title="Совершить покупку в один клик" style="text-decoration: none;" class="ShAA_productDesigner" href="/index.php?module=cart&one_click&clear_template&oneclick_product={$product->product_id}" rel="facebox" target="_blank" id="edit_link" onclick="{literal}rG('BUY_1_CLICK_SITE');{/literal}">
                                    <div class="ShAA_discountButton">
                                        Купить
                                    </div>
                                </a>
	{else}
...
jQuery(document).ready(function() {
    ...
    if (typeof(dataLayer) !== 'undefined' && dataLayer) { // Коллектор данных для ecommerse
        ...
        ecommerce['purchase']     = {"actionField": { "id"      : "S{/literal}{$purchase_data.order_id}{literal}",
                                                      "goal_id" : "10066135"},
                                     "products": [product]};
