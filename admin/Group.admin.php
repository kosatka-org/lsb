...
class Group extends Widget
{
  ...
  function prepare()
  {
  	   ...
  	   isset($_POST['discount']))
  	{
  		...
  		$this->group->show_all_products = $_POST['show_all_products'] ? 1 : 0;
        ...
        else
  		{
  			...
  			$query = sql_placeholder('UPDATE groups SET name=?, discount=?, show_all_products=? where group_id=?',
  			                          $this->group->name,
  			                          $this->group->discount,
  			                          $this->group->show_all_products,
  			                          $group_id);
