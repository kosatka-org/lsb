  ...
  function prepare()
  {
  	if(isset($_GET['set_main']))
  	{
        ...
        if($this->param('recalculate')==1)
        {
			...
			$this->db->query("UPDATE products SET price=price*$coef");        
			$this->db->query("UPDATE products SET old_price=old_price*$coef");        
      ...
      $code       =  $_POST['code'];
  		...
  		$query = "INSERT INTO currencies (name, eng_name, rate_from, rate_to, sign, code) VALUES('$name', '$eng_name', '$rate_from', '$rate_to', '$sign', '$code')";
  		$this->db->query($query);
    ...
    if(isset($_POST['names']))
    {
      ...
      {
        $eng_name   =  $_POST['eng_names'][$id];
        $rate_from  =  $_POST['rates_from'][$id];
        $rate_to    =  $_POST['rates_to'][$id];
        $sign       =  $_POST['signs'][$id];
        $code       =  $_POST['codes'][$id];
        $this->db->query("UPDATE currencies SET name='$name', eng_name='$eng_name', sign='$sign', code='$code', rate_from='$rate_from', rate_to='$rate_to' WHERE currency_id='$id'");
      }
      if(isset($_GET['from']))
        header("Location: ".$_GET['from']);            
    }
