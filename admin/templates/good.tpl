    ...
    <tr>
      <td>
        <p>
          ...
          {* Текущая категория *}
          {if $good}{$good->title}{else}создать{/if}
  ...
  <div id="cont_border">
    <div id="cont">
      ...
      <div id="cont_top">
	    ...
	    <!-- Заголовок раздела /-->
        <h1 id="headline"> {if $good}{$good->title}{else}Создать{/if}</h1>
        <!-- /Заголовок раздела /-->
      ...
      <div id="cont_center">
        ...
        <!-- Форма товара #Begin /-->
					...
					<div id="over_left">	
							<table>
								<tr>
									...
									<td class="m_t"><p><input name="title" type="text" class="input3" value='{$good->title|escape}' format='.+' notice='{$Lang->ENTER_NAME}'/> <nobr><input name=visible type="checkbox" class="checkbox" {if $good->visible == 1 or !$good}checked{/if} value='1'/><span class="akt">Активна</span></nobr></p></td>
								</tr>
								<tr>
									<td class="model">Бренд</td>
									<td class="m_t"><p>
										<select name=brand_id class="select1">
											{foreach item=brand from=$Brands}
												<option value="{$brand->brand_id}" title="{$brand->url}" {if $brand->brand_id == $good->brand_id}selected{/if} >{$brand->name}</option>
											{/foreach}
										</select>
									</p></td>
								</tr>
										...
										<select name=category_id class="select1">
											...
											{foreach from=$Categories item=category}
												{if $CurrentCategory != $category->category_id}
													<option value='{$category->category_id}' {if $category->category_id == $good->category_id}selected{/if} category_name='{$category->single_name}'  title="{$category->url}">{section name=sp loop=$level}&nbsp;&nbsp;&nbsp;&nbsp;{/section}{$category->name}</option>
								...
								<tr>
									<td class="model">Аннотация</td>
									<td class="m_t"><p><textarea name="annotation" style="height:100px"  class="input3">{$good->annotation|escape|nl2br}</textarea></p></td>
								</tr>
							</table>
							...
							<div class="gray_block">
								...
								<tr>
									...
									<td class="m_t"><p><input name="url" type="text" class="input6" value='{$good->url}'/></p></td>
								</tr>
								<tr>
									<td class="model2">Meta Title</td>
									<td class="m_t"><p><input name="meta_title"  type="text" class="input6" value='{$good->meta_title}' /></p></td>
								</tr>
								<tr>
									<td class="model2">Meta Keywords</td>
									<td class="m_t"><p><input name="meta_keywords" type="text" class="input6" value='{$good->meta_keywords}' /></p></td>
								</tr>
								<tr>
									<td class="model2">Meta Description</td>
									<td class="m_t"><p><input name="meta_description" type="text" class="input6" value='{$good->meta_description}'/></p></td>
								</tr>
							</table>
							</div>
							...
							<table>
								<tr>
									<td>
									    ...
									    {if $good->image}
										<img id=image class="image_preview" src='../files/goods/{$good->image}?r={math equation="rand(1,1000)"}' alt=""/>
										<p><img src="./images/cancel1.jpg" alt=""/><a href="#" class="link" onclick="javascript: window.document.getElementById('image').src='images/no_photo.jpg'; window.document.good.delete_image.value = 1; return false;">Удалить</a></p>
										{else}
				...
				<div class="area">
					...
					{if $is_copywriter AND !empty($good)}
						{if !((empty($check_task->text) OR (in_array($check_task->text->status, array('new', 'declined', 'need_check')) AND in_array($check_task->text->copywriter_id, array(0, $smarty.session.user->user_id)))) AND empty($good->text))}<span style="color:#f00;"> - не доступно для копирайтера</span>{/if}
					{/if}</span>
					{if $is_copywriter AND !empty($good) AND $check_task->text->status == 'declined' AND !empty($check_task->text->decline_reason)}
						<p>Причина отказа: {$check_task->text->decline_reason}</p>
					{/if}
					<p><textarea id="description" name="text" class="editor_small">{if $good->text}{$good->text}{else}{$check_task->text->text}{/if}</textarea></p>
				</div>
