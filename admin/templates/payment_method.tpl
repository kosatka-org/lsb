        ...
        <!-- Форма товара #Begin /-->
					...
					<div id="over_left">	
							<table>
								...
								<tr>
									...
									<td class="m_t"><p><input name="eng_name" type="text" class="input3" value='{$Item->eng_name|escape}' format='.+' notice='{$Lang->ENTER_NAME}'/>
									</p></td>
								</tr>
              ...
              <select name=currency_id class='select1'>
                ...
                {/foreach}
									...
									<br><nobr><input name=enabled type="checkbox" class="checkbox" {if $Item->enabled}checked{/if} value='1'/><span class="akt">Активна на сайте</span></nobr> &nbsp; &nbsp;<br>
									<br><nobr><input name=enabled_admin type="checkbox" class="checkbox" {if $Item->enabled_admin}checked{/if} value='1'/><span class="akt">Активна в админке</span></nobr> &nbsp; &nbsp;<br>
									<nobr><input name=is_local type="checkbox" class="checkbox" {if $Item->is_local}checked{/if} value='1'/><span class="akt">Локальная оплата</span></nobr> &nbsp; &nbsp;
									</p></td>
								</tr>
								<tr>
...
<!--										<OPTION value='rbk_money' {if $Item->module=='rbk_money'}selected{/if}>RBK Money</OPTION>
										<OPTION value='ya_money' {if $Item->module=='ya_money'}selected{/if}>Yandex Деньги</OPTION>-->
										<OPTION value='visa_mc' {if $Item->module=='visa_mc'}selected{/if}>Visa/MasterCard</OPTION>
										<OPTION value='rfi_payment' {if $Item->module=='rfi_payment'}selected{/if}>БАНК РФИ - Visa/MasterCard</OPTION>
							...
							<table>
								<tr>
									<td>
										...
										{if $Item->image}
										<img id=image class="image_preview" src='/files/payments/{$Item->image}' alt=""/>
									...
									<td class="pad_l">
										<p><input type="file" name="image" class="picture"/></p>
...
<!-- Content #End /--> 
          ...
          {literal} 
            ...
            {
              all_divs = new Array('webmoney', 'robokassa', 'assist', 'upc', 'receipt');
