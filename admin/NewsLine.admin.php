...
class NewsLine extends Widget {
  	...
  	if((isset($_POST['act']) && $_POST['act']=='delete' || isset($_GET['act']) && $_GET['act']=='delete') && (isset($_POST['items']) || isset($_GET['item_id']) ))
  	{
        ...
        if(isset($_GET['item_id']) && !empty($_GET['item_id']))
  		...
  		if(is_array($items))
  		  ...
  		  $items_sql = $items;
  		$query = "DELETE FROM news WHERE news.news_id IN ('$items_sql')";
  		$this->db->query($query);
  ...
  function fetch() {
    ...
    $this->db->query("SELECT SQL_CALC_FOUND_ROWS n.*, u.name, DATE_FORMAT(n.date, '%d.%m.%Y') as add_date
    				    FROM news n
					    LEFT JOIN users u ON n.editor_id = u.user_id
