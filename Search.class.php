...
class Search extends Widget
{
	...
	{	 
		...
		if (!empty($s))
		{
			$query = 'SELECT products.*, categories.name as category, categories.url as category_url, brands.name as brand, brands.url as brand_url, products.price*(100-'.(isset($this->user->discount)?$this->user->discount:0).")/100 as discount_price  FROM categories, products LEFT JOIN brands ON brands.brand_id = products.brand_id WHERE categories.category_id = products.category_id AND categories.enabled=1 AND products.enabled=1 $s GROUP BY products.product_id ORDER BY products.order_num DESC LIMIT $this->max";
			$this->db->query($query);
			$products = $this->db->results();
			if($this->db->num_rows() == 1)
			{
				$product_url = $products[0]->url;
				header('Location: https://'.$this->root_url.'/products/'.$product_url);
				exit();			
			}
		}
		else
